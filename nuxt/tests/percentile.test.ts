...
import { calculateCommunityPercentile, type WordStatsData } from '../utils/stats';

function makeStats(distribution: Record<number, number>, losses: number = 0): WordStatsData {
...
describe('calculateCommunityPercentile', () => {
    ...
    it('should return null for null-ish stats', () => {
        expect(calculateCommunityPercentile(3, null as unknown as WordStatsData)).toBeNull();
    });

    it('should handle missing distribution keys gracefully', () => {
        const stats: WordStatsData = { total: 5, losses: 2, distribution: { '3': 3 } };
