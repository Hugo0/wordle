...
import { defineComponent, withAsyncContext, unref, mergeProps, ref, computed, withCtx, createTextVNode, createVNode, toDisplayString, resolveComponent, useSSRContext } from 'vue';
...
const useLanguageStore = defineStore("language", () => {
  ...
  const rightToLeft = computed(() => {
    var _a;
    return ((_a = config.value) == null ? void 0 : _a.right_to_left) === "true";
  });
  const graphemeMode = computed(() => {
    var _a;
    return ((_a = config.value) == null ? void 0 : _a.grapheme_mode) === "true";
  });
  const languageCode = computed(() => {
    var _a, _b;
    return (_b = (_a = config.value) == null ? void 0 : _a.language_code) != null ? _b : "";
  });
...
const useGameStore = defineStore("game", () => {
  ...
  function addChar(char) {
    ...
    if (row && rowClasses) {
      ...
      const displayChar = toFinalForm(char, isLastPosition, (_a = lang.config) != null ? _a : {});
  ...
  function updateColors() {
    ...
    for (let i = 0; i < row.length; i++) {
      const guessChar = row[i];
      if (!guessChar || ((_a = classes[i]) == null ? void 0 : _a.includes("correct"))) continue;
  ...
  function getEmojiBoard() {
    const settings = useSettingsStore();
    let board = "";
    const greenEmoji = settings.highContrast ? "\u{1F7E6}" : "\u{1F7E9}";
    const yellowEmoji = settings.highContrast ? "\u{1F7E7}" : "\u{1F7E8}";
  ...
  function getShareText() {
    ...
    const lang = useLanguageStore();
    const settings = useSettingsStore();
    const name = ((_a = lang.config) == null ? void 0 : _a.name_native) || ((_b = lang.config) == null ? void 0 : _b.language_code) || "";
  ...
  function checkHardMode(guess) {
    ...
    for (let r = 0; r < activeRow.value; r++) {
      ...
      for (let c = 0; c < row.length; c++) {
        ...
        if (!letter) continue;
        if (tileClass.includes("correct") && !tileClass.includes("semicorrect") && !tileClass.includes("incorrect")) {
          if (((_a = guess[c]) == null ? void 0 : _a.toLowerCase()) !== letter.toLowerCase()) {
      ...
      ssrRenderList(__props.keyboard, (row, i) => {
        ...
        ssrRenderList(row, (key) => {
          ...
          _push(ssrRenderComponent(_component_KeyboardKey, {
            ...
            hint: (_a = __props.hints[key.toLowerCase()]) == null ? void 0 : _a.text,
            "hint-above": (_b = __props.hints[key.toLowerCase()]) == null ? void 0 : _b.above,
...
const _sfc_main$4 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const help = computed(() => {
      var _a, _b;
      return (_b = (_a = lang.config) == null ? void 0 : _a.help) != null ? _b : {};
    });
    const exampleWord1 = computed(() => {
      var _a;
      return [...(_a = lang.wordList[0]) != null ? _a : ""];
    });
    const exampleWord2 = computed(() => {
      var _a;
      return [...(_a = lang.wordList[1]) != null ? _a : ""];
    });
    const exampleWord3 = computed(() => {
      var _a;
      return [...(_a = lang.wordList[2]) != null ? _a : ""];
    });
    return (_ctx, _push, _parent, _attrs) => {
      var _a, _b, _c, _d, _e;
      _push(`<div${ssrRenderAttrs(mergeProps({
        ...
        style: __props.visible ? null : { display: "none" }
      }))}><div class="modal-animate bg-white dark:bg-neutral-800 rounded-lg shadow-lg p-5 m-4 border-2 border-slate-200 dark:border-neutral-600 mx-auto w-full max-w-md sm:max-w-lg"><div class="flex flex-col gap-2 relative"><button type="button" aria-label="Close" class="absolute top-0 right-0 p-1 ml-auto z-50"><span class="leading-[0.25] h-5 w-5 text-3xl text-neutral-400 block outline-none focus:outline-none">\xD7</span></button><h2 class="flex mx-auto uppercase font-bold text-2xl tracking-wider"> Wordle <span class="ml-1 text-transparent bg-clip-text bg-gradient-to-r from-cyan-500 to-blue-500">${ssrInterpolate((_a = unref(lang).config) == null ? void 0 : _a.name_native)}</span></h2><h3 class="font-bold text-lg">${ssrInterpolate(help.value.title)}</h3><p class="text-sm">${ssrInterpolate(help.value.text_1_1_1)} <a class="font-bold uppercase">Wordle</a> ${ssrInterpolate(help.value.text_1_1_2)}</p><p class="text-sm">${ssrInterpolate(help.value.text_1_2)}</p><p class="text-sm">${ssrInterpolate(help.value.text_1_3)}</p><div class="border-t-2 border-gray-300 dark:border-gray-600"></div><div class="justify-center items-center flex flex-col gap-2"><h2 class="text-md font-semibold text-gray-900 dark:text-gray-100">${ssrInterpolate(help.value.title_2)}</h2>`);
      ...
      _push(`</div><div class="border-t-2 border-gray-300 dark:border-gray-600"></div><div class="flex flex-col gap-2 py-2"><a href="https://github.com/Hugo0/wordle/issues" target="_blank" rel="noopener noreferrer" class="flex items-center gap-2 text-sm text-neutral-600 dark:text-neutral-300 hover:text-neutral-900 dark:hover:text-white"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M9 19c-4.3 1.4 -4.3 -2.5 -6 -3m12 5v-3.5c0 -1 .1 -1.4 -.5 -2c2.8 -.3 5.5 -1.4 5.5 -6a4.6 4.6 0 0 0 -1.3 -3.2a4.2 4.2 0 0 0 -.1 -3.2s-1.1 -.3 -3.5 1.3a12.3 12.3 0 0 0 -6.2 0c-2.4 -1.6 -3.5 -1.3 -3.5 -1.3a4.2 4.2 0 0 0 -.1 3.2a4.6 4.6 0 0 0 -1.3 3.2c0 4.6 2.7 5.7 5.5 6c-.6 .6 -.6 1.2 -.5 2v3.5"></path></svg> ${ssrInterpolate(((_c = (_b = unref(lang).config) == null ? void 0 : _b.ui) == null ? void 0 : _c.report_issue) || "Report an Issue")}</a><a href="https://github.com/Hugo0/wordle" target="_blank" rel="noopener noreferrer" class="flex items-center gap-2 text-sm text-neutral-600 dark:text-neutral-300 hover:text-neutral-900 dark:hover:text-white"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M10 14a3.5 3.5 0 0 0 5 0l4 -4a3.5 3.5 0 0 0 -5 -5l-.5 .5"></path><path d="M14 10a3.5 3.5 0 0 0 -5 0l-4 4a3.5 3.5 0 0 0 5 5l.5 -.5"></path></svg> ${ssrInterpolate(((_e = (_d = unref(lang).config) == null ? void 0 : _d.ui) == null ? void 0 : _e.view_source) || "View Source Code")}</a></div><div class="border-t-2 border-gray-300 dark:border-gray-600"></div><button class="uppercase font-bold text-sm tracking-wider">${ssrInterpolate(help.value.close)} <span class="text-lg text-neutral-400">\xD7</span></button></div></div></div>`);
    };
  }
});
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "fixed top-10 left-0 w-full h-full z-30 items-center mx-auto flex" }, _attrs, {
        style: __props.visible ? null : { display: "none" }
      }))}><div class="modal-animate bg-white dark:bg-neutral-800 rounded-lg shadow-lg p-5 m-4 border-2 border-slate-200 dark:border-neutral-600 mx-auto w-full max-w-xs sm:max-w-md"><div class="flex flex-col gap-2 relative"><button type="button" aria-label="Close" class="absolute top-0 right-0 p-1 ml-auto z-50"><span class="leading-[0.25] h-5 w-5 text-3xl text-neutral-400 block outline-none focus:outline-none">\xD7</span></button><h3 class="uppercase font-bold text-xl tracking-wider mb-5">${ssrInterpolate(((_b = (_a = unref(lang).config) == null ? void 0 : _a.ui) == null ? void 0 : _b.settings) || "Settings")}</h3><div class="space-y-4"><div class="flex flex-row items-center"><div class="flex-grow"><p>${ssrInterpolate(((_d = (_c = unref(lang).config) == null ? void 0 : _c.ui) == null ? void 0 : _d.dark_mode) || "Dark Mode")}</p></div>`);
      ...
      _push(`</div><div class="border-t-2 border-gray-300 dark:border-gray-600"></div><div class="flex flex-row items-center"><div class="flex-grow"><p>${ssrInterpolate(((_f = (_e = unref(lang).config) == null ? void 0 : _e.ui) == null ? void 0 : _f.sound_and_haptics) || "Sound & Haptics")}</p></div>`);
      _push(ssrRenderComponent(_component_ToggleSwitch, {
        "model-value": unref(settings).feedbackEnabled,
        "onUpdate:modelValue": ($event) => unref(settings).toggleFeedback()
      }, null, _parent));
      _push(`</div><div class="border-t-2 border-gray-300 dark:border-gray-600"></div><div class="flex flex-row items-center"><div class="flex-grow"><p>${ssrInterpolate(((_h = (_g = unref(lang).config) == null ? void 0 : _g.ui) == null ? void 0 : _h.word_info) || "Word Info")}</p><p class="text-xs text-neutral-500 dark:text-neutral-400">${ssrInterpolate(((_j = (_i = unref(lang).config) == null ? void 0 : _i.ui) == null ? void 0 : _j.word_info_desc) || "Definition & AI art after game")}</p></div>`);
      _push(ssrRenderComponent(_component_ToggleSwitch, {
        "model-value": unref(settings).wordInfoEnabled,
        "onUpdate:modelValue": ($event) => unref(settings).toggleWordInfo()
      }, null, _parent));
      _push(`</div><div class="border-t-2 border-gray-300 dark:border-gray-600"></div><div><p class="text-sm font-semibold mb-2">${ssrInterpolate(((_l = (_k = unref(lang).config) == null ? void 0 : _k.ui) == null ? void 0 : _l.difficulty) || "Difficulty")}</p><div class="flex rounded-lg overflow-hidden border border-neutral-300 dark:border-neutral-600"><button type="button" class="${ssrRenderClass([
      ...
      _push(`</div><div class="border-t-2 border-gray-300 dark:border-gray-600"></div><div class="flex flex-row items-center"><p class="flex-grow">${ssrInterpolate(((_n = (_m = unref(lang).config) == null ? void 0 : _m.ui) == null ? void 0 : _n.language) || "Language")}</p>`);
      _push(ssrRenderComponent(_component_NuxtLink, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          var _a2, _b2, _c2, _d2;
          if (_push2) {
            _push2(`${ssrInterpolate(((_b2 = (_a2 = unref(lang).config) == null ? void 0 : _a2.ui) == null ? void 0 : _b2.change_language) || "Change Language")}`);
          } else {
            return [
              createTextVNode(toDisplayString(((_d2 = (_c2 = unref(lang).config) == null ? void 0 : _c2.ui) == null ? void 0 : _d2.change_language) || "Change Language"), 1)
            ];
          }
        }),
        _: 1
      ...
      if (Object.keys(unref(lang).keyboardLayouts).length > 1) {
        _push(`<!--[--><div class="border-t-2 border-gray-300 dark:border-gray-600"></div><div class="flex flex-row items-center"><p class="flex-grow">${ssrInterpolate(((_p = (_o = unref(lang).config) == null ? void 0 : _o.ui) == null ? void 0 : _p.keyboard_layout) || "Keyboard layout")}</p><select${ssrRenderAttr("value", unref(lang).activeLayout)} class="border border-neutral-300 dark:border-neutral-600 rounded px-2 py-1 bg-white dark:bg-neutral-700 dark:text-white"><!--[-->`);
...
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    function getDistributionBarWidth(n) {
      ...
      const count = (_a = distribution[n]) != null ? _a : 0;
    ...
    function getLanguageName(code) {
      ...
      if (code === lang.languageCode) {
        return ((_a = lang.config) == null ? void 0 : _a.name_native) || ((_b = lang.config) == null ? void 0 : _b.name) || code;
      }
      return code;
    }
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "fixed top-10 left-0 w-full h-full z-50 items-center flex" }, _attrs, {
        style: __props.visible ? null : { display: "none" }
      }))}><div class="modal-animate relative mx-auto w-full max-w-lg z-50 border-2 border-slate-200 dark:border-neutral-600 rounded-xl max-h-[85vh] overflow-y-auto"><div class="border-0 rounded-lg drop-shadow-xl relative flex flex-col w-full bg-white dark:bg-neutral-800 outline-none focus:outline-none"><div class="flex-initial relative mx-5 mt-5 z-50"><h3 class="flex-auto text-center text-xl font-bold"> Wordle ${ssrInterpolate((_a = unref(lang).config) == null ? void 0 : _a.name_native)} #${ssrInterpolate(unref(lang).todaysIdx)} ${ssrInterpolate(unref(game).attempts)}/6 </h3><button type="button" aria-label="Close" class="absolute top-0 right-0 p-1 ml-auto z-50"><span class="leading-[0.25] h-5 w-5 text-3xl text-neutral-400 block outline-none focus:outline-none"> \xD7 </span></button></div><div class="flex mx-5 mt-2 border-b border-gray-200 dark:border-gray-600" role="tablist"><button role="tab"${ssrRenderAttr("aria-selected", statsTab.value === "today")} class="${ssrRenderClass([
      ...
      ])}">${ssrInterpolate(((_c = (_b = unref(lang).config) == null ? void 0 : _b.ui) == null ? void 0 : _c.today) || "Today")}</button><button role="tab"${ssrRenderAttr("aria-selected", statsTab.value === "stats")} class="${ssrRenderClass([
        ...
        "flex-1 py-2 text-xs font-medium transition-colors border-b-2"
      ])}">${ssrInterpolate(((_e = (_d = unref(lang).config) == null ? void 0 : _d.ui) == null ? void 0 : _e.statistics) || "Stats")}</button><button role="tab"${ssrRenderAttr("aria-selected", statsTab.value === "global")} class="${ssrRenderClass([
        ...
        "flex-1 py-2 text-xs font-medium transition-colors border-b-2"
      ])}">${ssrInterpolate(((_g = (_f = unref(lang).config) == null ? void 0 : _f.ui) == null ? void 0 : _g.all_languages) || "All Languages")}</button></div><div style="${ssrRenderStyle(statsTab.value === "today" ? null : { display: "none" })}">`);
      ...
      if (unref(game).communityPercentile !== null) {
        ...
        if (unref(game).communityTotal <= 1) {
          ...
          _push(`<!--[-->${ssrInterpolate(((_i = (_h = unref(lang).config) == null ? void 0 : _h.ui) == null ? void 0 : _i.better_than) || "Better than")} ${ssrInterpolate(unref(game).communityPercentile)}% ${ssrInterpolate(((_k = (_j = unref(lang).config) == null ? void 0 : _j.ui) == null ? void 0 : _k.of_players) || "of players")}<!--]-->`);
        }
      ...
      if (unref(game).shareButtonState === "success") {
        _push(`<span> \u2713 ${ssrInterpolate(((_m = (_l = unref(lang).config) == null ? void 0 : _l.text) == null ? void 0 : _m.copied) || "Copied!")}</span>`);
      } else {
        _push(`<span>${ssrInterpolate((_o = (_n = unref(lang).config) == null ? void 0 : _n.text) == null ? void 0 : _o.share)}</span>`);
      }
      _push(`</button></div><div class="flex flex-col items-center py-2 px-6 border-t border-gray-200 dark:border-gray-600"><p class="uppercase font-semibold text-xs text-neutral-500 dark:text-neutral-400">${ssrInterpolate((_q = (_p = unref(lang).config) == null ? void 0 : _p.text) == null ? void 0 : _q.next_word)}</p><p class="text-2xl font-bold">${(_r = unref(game).timeUntilNextDay) != null ? _r : ""}</p></div></div><div style="${ssrRenderStyle(statsTab.value === "stats" ? null : { display: "none" })}"><div class="px-6 py-3"><h4 class="text-xs font-semibold uppercase tracking-wide mb-1 text-center text-neutral-500 dark:text-neutral-400">${ssrInterpolate(((_t = (_s = unref(lang).config) == null ? void 0 : _s.ui) == null ? void 0 : _t.guess_distribution) || "Guess Distribution")}</h4><div class="space-y-0.5"><!--[-->`);
      ssrRenderList(6, (n) => {
      ...
      _push(`<!--]--></div></div><div class="px-4 py-3 border-t border-gray-200 dark:border-gray-600"><div class="grid grid-cols-4 gap-1"><div class="text-center"><p class="text-xl font-bold">${ssrInterpolate(unref(statsStore).stats.n_games)}</p><p class="text-[10px] text-neutral-500 dark:text-neutral-400">${ssrInterpolate(((_v = (_u = unref(lang).config) == null ? void 0 : _u.ui) == null ? void 0 : _v.games) || "Games")}</p></div><div class="text-center"><p class="text-xl font-bold">${ssrInterpolate(Math.round(unref(statsStore).stats.win_percentage))}% </p><p class="text-[10px] text-neutral-500 dark:text-neutral-400">${ssrInterpolate(((_x = (_w = unref(lang).config) == null ? void 0 : _w.ui) == null ? void 0 : _x.win_percent) || "Win %")}</p></div><div class="text-center"><p class="text-xl font-bold">`);
      if (unref(statsStore).stats.current_streak > 0) {
        _push(`<span> \u{1F525} </span>`);
      } else {
        _push(`<!---->`);
      }
      _push(` ${ssrInterpolate(unref(statsStore).stats.current_streak)}</p><p class="text-[10px] text-neutral-500 dark:text-neutral-400">${ssrInterpolate(((_z = (_y = unref(lang).config) == null ? void 0 : _y.ui) == null ? void 0 : _z.streak) || "Streak")}</p></div><div class="text-center"><p class="text-xl font-bold">${ssrInterpolate(unref(statsStore).stats.longest_streak)}</p><p class="text-[10px] text-neutral-500 dark:text-neutral-400">${ssrInterpolate(((_B = (_A = unref(lang).config) == null ? void 0 : _A.ui) == null ? void 0 : _B.best) || "Best")}</p></div></div></div><div class="relative flex-auto mx-6 my-3 pt-3 border-t border-gray-200 dark:border-gray-600"><p id="emoji_board" class="text-md text-center whitespace-pre-line">${(_C = unref(game).emojiBoard) != null ? _C : ""}</p>`);
      if (unref(game).attempts === "0") {
        _push(`<p class="text-center text-sm text-gray-600 dark:text-gray-400">${ssrInterpolate((_E = (_D = unref(lang).config) == null ? void 0 : _D.text) == null ? void 0 : _E.no_attempts)}</p>`);
      } else {
        _push(`<!---->`);
      }
      ...
      if (unref(game).shareButtonState === "success") {
        _push(`<span> \u2713 ${ssrInterpolate(((_G = (_F = unref(lang).config) == null ? void 0 : _F.text) == null ? void 0 : _G.copied) || "Copied!")}</span>`);
      } else {
        _push(`<span>${ssrInterpolate((_I = (_H = unref(lang).config) == null ? void 0 : _H.text) == null ? void 0 : _I.share)}</span>`);
      }
      _push(`</button></div></div><div style="${ssrRenderStyle(statsTab.value === "global" ? null : { display: "none" })}"><div class="px-4 py-3"><div class="grid grid-cols-4 gap-1 mb-2"><div class="text-center"><p class="text-xl font-bold">${ssrInterpolate(unref(statsStore).totalStats.total_games)}</p><p class="text-[10px] text-neutral-500 dark:text-neutral-400">${ssrInterpolate(((_K = (_J = unref(lang).config) == null ? void 0 : _J.ui) == null ? void 0 : _K.games) || "Games")}</p></div><div class="text-center"><p class="text-xl font-bold">${ssrInterpolate(Math.round(
        unref(statsStore).totalStats.total_win_percentage
      ))}% </p><p class="text-[10px] text-neutral-500 dark:text-neutral-400">${ssrInterpolate(((_M = (_L = unref(lang).config) == null ? void 0 : _L.ui) == null ? void 0 : _M.win_percent) || "Win %")}</p></div><div class="text-center"><p class="text-xl font-bold">`);
      if (unref(statsStore).totalStats.current_overall_streak > 0) {
        _push(`<span> \u{1F525} </span>`);
      } else {
        _push(`<!---->`);
      }
      _push(` ${ssrInterpolate(unref(statsStore).totalStats.current_overall_streak)}</p><p class="text-[10px] text-neutral-500 dark:text-neutral-400">${ssrInterpolate(((_O = (_N = unref(lang).config) == null ? void 0 : _N.ui) == null ? void 0 : _O.streak) || "Streak")}</p></div><div class="text-center"><p class="text-xl font-bold">${ssrInterpolate(((_P = unref(statsStore).totalStats.languages_won) == null ? void 0 : _P.length) || 0)}</p><p class="text-[10px] text-neutral-500 dark:text-neutral-400">${ssrInterpolate(((_R = (_Q = unref(lang).config) == null ? void 0 : _Q.ui) == null ? void 0 : _R.languages) || "Languages")}</p></div></div>`);
      if (Object.keys(
        ...
        ssrRenderList(unref(statsStore).totalStats.game_stats, (langStats, code) => {
          _push(`<div class="flex items-center justify-between py-1 text-xs"><span class="font-medium">${ssrInterpolate(getLanguageName(code))}</span><span class="text-neutral-500 dark:text-neutral-400">${ssrInterpolate(langStats.n_games)} \xB7 ${ssrInterpolate(Math.round(langStats.win_percentage))}% `);
          if (langStats.current_streak > 0) {
            _push(`<span class="text-orange-500 ml-1"> \u{1F525}${ssrInterpolate(langStats.current_streak)}</span>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</span></div>`);
        ...
        _push(`<p class="text-xs text-neutral-500 dark:text-neutral-400 text-center pt-1 border-t border-neutral-200 dark:border-neutral-600">${ssrInterpolate(((_T = (_S = unref(lang).config) == null ? void 0 : _S.ui) == null ? void 0 : _T.play_more_languages) || "Play more languages to see your global stats!")}</p>`);
      }
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    useSeoMeta({
      title: `${wordleTitle} \u2014 ${((_a = config.meta) == null ? void 0 : _a.title) || "Daily Word Puzzle"}`,
      description: ((_b = config.meta) == null ? void 0 : _b.description) || `Play Wordle in ${config.name}. Guess the daily 5-letter word!`,
      ogTitle: wordleTitle,
      ogDescription: ((_c = config.meta) == null ? void 0 : _c.description) || `Play Wordle in ${config.name}`,
      ogUrl: `https://wordle.global/${lang}`,
      ogType: "website",
      ogLocale: ((_d = config.meta) == null ? void 0 : _d.locale) || lang,
      ogImage: `https://wordle.global/images/share/${lang}_1.png`,
      twitterCard: "summary_large_image"
    });
    useHead({
      htmlAttrs: {
        lang: ((_f = (_e = config.meta) == null ? void 0 : _e.locale) == null ? void 0 : _f.split("_")[0]) || lang,
...
//# sourceMappingURL=index-BH-xFwCE.mjs.map
