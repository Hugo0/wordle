        ...
        while i < len(lines):
            ...
            if opt == "PFX" or opt == "SFX":
                ...
                while j < num_entries:
                    ...
                    self.affix_rules[flag].append(
                        AffixRule(flag, opt, combine, char_to_strip, affix, condition)
                    )
    ...
    def generate_json(self, out_file, gzip_set):
        ...
        if self.rep_table:
            result += (
                ...
                + tab
                + '"repTable": '
                + json.dumps(self.aff.rep_table, separators=(",", ":"))
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generates JSON formatted dictionary based on hunspell and priority"
    )
    parser.add_argument("-o", "--output", help="Output name of JSON")
    parser.add_argument(
        "-f",
        "--format",
        help="Format of the derivatives of each baseword (full|addsub) [Default=full]",
        default="full",
    )
    parser.add_argument(
        "-k",
        "--key",
        action="store_true",
        help="If format is addsub, list of character removals and affixes can be generated to reduce redudant prefixes and suffixes among base words",
    )
    parser.add_argument(
        "-g",
        "--gzip",
        action="store_true",
        help="To reduce generated JSON file further, you can compress it using gzip which most modern browsers can decompress now.",
    )
    parser.add_argument(
        "--noCompounds",
        action="store_false",
        help="If you do not want to generate the compound regular expressions.",
    )
    parser.add_argument(
        "-r",
        "--rep_table",
        action="store_true",
        help="If you want to add the Hunspell replacement table to generated JSON output",
    )
    parser.add_argument(
        "-p",
        "--pretty",
        action="store_true",
        help="Output JSON format with formal indentation and line breaks",
    )
    parser.add_argument(
        "dictionary",
        nargs="+",
        help="Name of dictionary (e.g. en_US) or individual .dic and .aff files.",
    )
