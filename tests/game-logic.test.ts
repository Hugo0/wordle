 ...
 *
 * Since calculateColors is embedded in the Pinia game store (not exported as a
 * pure function), we extract an equivalent standalone implementation here for
 * unit testing. The algorithm is identical to the one in stores/game.ts
 * updateColors().
 *
 * calculateStats is tested by exercising the same algorithm that lives in
 * stores/stats.ts. We keep a local copy here to test the pure logic without
 * needing Pinia/Vue setup.
 */
import { describe, it, expect } from 'vitest';

/**
 * Color calculation algorithm - extracted from stores/game.ts updateColors()
...
/**
 * Stats calculation - mirrors stores/stats.ts calculateStats()
 */
...
describe('calculateColors', () => {
    ...
    it('should prioritize green over yellow for duplicate letters', () => {
        ...
        // "aback" = a,b,a,c,k - two 'a's at positions 0 and 2
        // "abaca" = a,b,a,c,a
        // pos 0: a=a correct
        // pos 1: b=b correct
        // pos 2: a=a correct
